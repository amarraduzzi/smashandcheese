...
export const FloatingCartBar: React.FC<FloatingCartBarProps> = ({ language }) => {
  ...
  return (
    // Visible at every viewport width — unlike the original standalone
    // engine (mobile-only there), this site has no desktop cart icon in
    // the header (Header.astro is static, outside the cart's React tree),
    // so the floating bar is the only way to reach the drawer on desktop
    // too. max-w-md keeps it from stretching edge-to-edge on wide screens.
    <div className="fixed inset-x-0 bottom-0 z-30 px-3 pointer-events-none" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
      <AnimatePresence>
